<div>
    <div class="mt-20 grid grid-cols-1 sm:grid-cols-2 gap-4" wire:loading.remove>
        <div class="col-span-2 w-full p-4 border border-gray-100 shadow rounded-lg">
            <div class="flex gap-3 mb-2">
                ...
                <h2 class="text-base font-medium">Income</h2>
            </div>
            <hr class="mb-4">
        ...
        <div class="w-full p-4 border border-gray-100 shadow rounded-lg">
            <div class="mb-2">
                <h3 class="text-base font-medium">Invoice</h3>
            </div>
            <hr class="mb-4">

            <div class="flex gap-2">
                <div class="w-full p-5 my-4 border border-gray-200 rounded-lg shadow">
                    <p>Bulan ini : <span class="font-bold" id="mounth">{{ number_format($countInvoice['count-month']) }}</span></p>
                </div>
                <div class="w-full p-5 my-4 border border-gray-200 rounded-lg shadow">
                    <p>Hari ini : <span class="font-bold" id="today">{{ number_format($countInvoice['count-today']) }}</span></p>
                </div>
            </div>
            <canvas id="count-invoice-chart"></canvas>
        </div>
...
<script>
            ...
            datasets: [
                {
                    ...
                    label: '',
                    data: {{ json_encode($income['income']) }}
                }
            ]
        }
    })
    ...
    // total invoice
    new Chart(document.getElementById('count-invoice-chart'),{
        type: 'bar',
        data: {
            labels: ['Januari', 'Febuari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'],
            datasets: [
                {
                    label: '',
                    data: {{ json_encode($countInvoice['count-all-month']['total']) }}
                }
            ]
        }
    })
